...
const ProductVariantQuickCreateForm = ({ open, onClose }) => {
  const { t } = useLocales();
  ...
  const onSubmit = () => {
    ...
    if (data.options?.length) {
      const index = data.options.indexOf(
        data.options.find((option) => !option.value)
      );

      if (index != -1) {
        setErrors({ ...errors, options: [index] });
    ...
    post(route(routes.dashboard.variants.store, [product.id]), {
      preserveScroll: true,
      onSuccess: () => {
...
ProductVariantQuickCreateForm.propTypes = {
  open: PropTypes.bool,
  onClose: PropTypes.func,
};

export default ProductVariantQuickCreateForm;
